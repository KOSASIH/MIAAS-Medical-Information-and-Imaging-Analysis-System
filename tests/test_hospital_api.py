...
class HospitalAPITestCase(unittest.TestCase):
    ...
    def test_create_hospital(self):
        response = self.client.post(
            "/hospitals", json={"name": "Hospital A", "location": "City A"}
        )
    ...
    def test_get_hospital(self):
        response = self.client.post(
            "/hospitals", json={"name": "Hospital B", "location": "City B"}
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_update_hospital(self):
        response = self.client.post(
            "/hospitals", json={"name": "Hospital C", "location": "City C"}
        )
        self.assertEqual(response.status_code, 201)
        hospital_id = json.loads(response.content)["id"]
        ...
        response = self.client.put(
            ...
            json={"name": "Hospital C", "location": "City C"},
        )
        self.assertEqual(response.status_code, 200)

    def test_delete_hospital(self):
        response = self.client.post(
            "/hospitals", json={"name": "Hospital D", "location": "City D"}
        )
        self.assertEqual(response.status_code, 201)
        hospital_id = json.loads(response.content)["id"]
