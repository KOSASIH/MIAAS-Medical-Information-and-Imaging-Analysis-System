...
class PatientAPITestCase(unittest.TestCase):
    ...
    def test_create_patient(self):
        response = self.client.post("/patients", json={"name": "John Doe", "age": 30, "gender": "Male"})
        self.assertEqual(response.status_code, 201)
        self.assertIn("id", json.loads(response.content))

    def test_get_patient(self):
        response = self.client.post("/patients", json={"name": "Jane Doe", "age": 25, "gender": "Female"})
        self.assertEqual(response.status_code, 201)
    ...
    def test_update_patient(self):
        ...
        response = self.client.put(f"/patients/{patient_id}", json={"name": "John Doe", "age": 31, "gender": "Male"})
        self.assertEqual(response.status_code, 200)
